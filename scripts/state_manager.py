...
ACTIVE_TASKS_AUTO_BEGIN = "<!-- AUTO-GENERATED-BEGIN: state_manager.py — programmatic tasks below -->"
ACTIVE_TASKS_AUTO_END = "<!-- AUTO-GENERATED-END -->"
...
def _compute_targets(conn: sqlite3.Connection,
    ...
    # ── ACTIVE_TASKS.md auto-generated section ──
    task_rows = conn.execute(
        "SELECT id, bucket, title, owner, priority, status, updated_at "
        "FROM active_task WHERE status IN ('open','blocked') "
        "ORDER BY bucket, priority ASC, updated_at DESC"
    ).fetchall()
    ...
    for bucket in sorted(by_bucket):
        rendered_tasks.append(f"\n### {bucket}\n")
        for row in by_bucket[bucket]:
            mark = "x" if row["status"] == "blocked" else " "
            tag = " 🚫 BLOCKED" if row["status"] == "blocked" else ""
            rendered_tasks.append(
                f"- [{mark}] #{row['id']} ({row['owner']}, p{row['priority']}) {row['title']}{tag}"
            )
    tasks_body_inner = "\n".join(rendered_tasks).strip() or "*No programmatic tasks — add via `state_manager task add`.*"
    tasks_body = (
        f"{ACTIVE_TASKS_AUTO_BEGIN}\n"
        f"<!-- regenerated by state_manager.py — open+blocked rows from active_task table -->\n\n"
        f"{tasks_body_inner}\n\n"
        f"{ACTIVE_TASKS_AUTO_END}"
    )
    tasks_before = ACTIVE_TASKS_MD.read_text(encoding="utf-8") if ACTIVE_TASKS_MD.exists() else ""
    if ACTIVE_TASKS_AUTO_BEGIN in tasks_before and ACTIVE_TASKS_AUTO_END in tasks_before:
        tasks_after = re.sub(
            re.escape(ACTIVE_TASKS_AUTO_BEGIN) + r".*?" + re.escape(ACTIVE_TASKS_AUTO_END),
            lambda _m: tasks_body,
            tasks_before,
            count=1,
            flags=re.DOTALL,
        )
    elif tasks_before:
        tasks_after = tasks_before.rstrip() + "\n\n" + tasks_body + "\n"
    else:
        tasks_after = (
            "# ACTIVE TASKS — Maven (CMO)\n\n"
            "> Programmatic rows below are managed by state_manager.py.\n"
            "> Hand-curated priority lists (P0/P1/P2) live outside the AUTO-GENERATED markers.\n\n"
            + tasks_body + "\n"
        )
    ...
    return {
        ...
        "active_tasks_md": {"before": tasks_before, "after": tasks_after, "rows": len(task_rows)},
    }
...
def export_active_tasks_md(conn: sqlite3.Connection | None = None) -> bool:
    """Rebuild the auto-generated section of memory/ACTIVE_TASKS.md."""
    own = conn is None
    conn = conn or connect(read_only=True)
    try:
        targets = _compute_targets(conn)
        tasks = targets["active_tasks_md"]
        if tasks["after"] != tasks["before"]:
            ACTIVE_TASKS_MD.parent.mkdir(parents=True, exist_ok=True)
            ACTIVE_TASKS_MD.write_text(tasks["after"], encoding="utf-8")
            return True
        return False
    finally:
        if own:
...
def export_check() -> dict:
    ...
    try:
        ...
        return {
            ...
            "active_tasks_md_drift": targets["active_tasks_md"]["before"] != targets["active_tasks_md"]["after"],
            "session_log_db_rows":   targets["session_log_md"]["rows"],
            "active_tasks_db_rows":  targets["active_tasks_md"]["rows"],
        }
    finally:
        conn.close()
