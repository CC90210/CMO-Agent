...
description: "Higgsfield image prompt director covering Banana Pro, Soul Cinema, and GPT-2. Three asset types in strict order — single-image character outfit on white seamless studio (Banana Pro or Soul Cinema two-step), 6-panel multi-angle character sheet off that base, and scene plates with/without characters. Plus GPT-2 for detail face/chest-up portraits. Reads reference images for hair, makeup, wardrobe, jewelry, identity. Outputs photorealistic prompts with locked hyperreal stack — skin pores, subsurface scattering, strand-by-strand hair, fabric weave, Kodak Vision3 film. Use for any character outfit reference, character sheet, model sheet, ref sheet, multi-angle sheet, scene plate, environment plate, face detail, or photorealistic still — even without saying 'Banana Pro,' 'Nano Banana,' or 'Soul Cinema.'"
tier: specialized
universal: true
triggers: ["banana pro prompt", "nano banana prompt", "soul cinema prompt", "higgsfield image", "character sheet", "model sheet", "ref sheet", "6-panel sheet", "character outfit reference", "scene plate", "environment plate", "face detail shot", "chest-up portrait", "photorealistic still", "GPT-2 prompt"]
dependencies: [image-generation, brand-guidelines]
related_assets: ["brain/brand-assets/oasis-ai/BRAND_SYSTEM.md"]
canon_references: [sutherland-signalling, dunford-positioning]
canon_source: brain/MARKETING_CANON.md
note: "Documentation-only skill. Requires Higgsfield platform access (Banana Pro, Soul Cinema, or GPT-2) to execute. Per CC's deferred AI capabilities memory, subscription is gated until GPU upgrade — the prompt-craft itself is valuable training before subscribe."
---
...
## MAVEN-SPECIFIC INTEGRATION NOTES

Added for the Maven CMO-Agent context (not part of the original skill).

**OASIS AI brand-lock (when generating OASIS visuals):** before composing Mode 3B environment plates or Mode 3A character-in-environment plates for OASIS content, pull the palette from `brain/brand-assets/oasis-ai/BRAND_SYSTEM.md`. Bake the hex values directly into the prompt for Mode 3B (`#0A1525` bg, `#1FE3F0` cyan accent, `#11202F` elevated, `#F0F4F8` off-white). For OASIS character/persona shots, prefer the M2 Studio/Editorial cinema grammar when picking from the Mode 3 camera table.

**Coexists with the existing image-generation skill:** [skills/image-generation/SKILL.md](skills/image-generation/SKILL.md) uses Google Gemini Nano Banana for fast/cheap ad creative (Meta feed, Stories, Google Display). Banana Pro Director is for premium Higgsfield work where character/wardrobe locking matters across many assets — character sheets, recurring brand personas, episodic content where identity has to survive across shots. Use Gemini for one-off ad imagery; use Banana Pro Director when building a character once and re-using them.

**Pairing with cinema-worldbuilder:** Mode 3 scene plates from this skill feed directly into Seedance prompts written via `cinema-worldbuilder`. Build the still in Banana Pro → upload it to Higgsfield as the visual reference → write the Seedance prompt with the matching cinema-mode camera grammar. Still and video share visual DNA.

**Subscription gate (memory: project_deferred_ai_capabilities.md):** Higgsfield (Banana Pro / Soul Cinema / GPT-2) subscription is currently gated by CC's GPU upgrade decision. This skill is installed as documentation; do not assume CC has an active Higgsfield account. If CC asks "actually generate this," confirm subscription status first or note the gate.
