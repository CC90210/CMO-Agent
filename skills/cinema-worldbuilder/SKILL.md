...
description: "Universal cinema worldbuilding director for Seedance video prompts. Five cinema modes (Narrative, Studio, Action, Performance, Atmospheric) with locked ARRI + Panavision/Cooke camera specs and diegetic-only audio rules. Reads reference images for wardrobe, hair, makeup, identity, and environment. Outputs production-ready Seedance prompts. Use whenever CC asks for a Seedance video prompt, mentions Seedance, asks for cinematic scene breakdowns, uploads reference images for a scene, describes a scene for video generation, or asks for shot prompts for music videos, action sequences, performance scenes, narrative shorts, fashion films, or atmospheric environment plates — even if 'cinematic' or a mode name isn't said."
tier: specialized
universal: true
triggers: ["seedance prompt", "cinematic prompt", "video prompt", "scene breakdown", "shot prompt", "music video prompt", "fashion film prompt", "environment plate", "atmospheric plate", "performance scene", "action scene", "narrative scene", "studio editorial", "cinema mode"]
dependencies: [banana-pro-director, elite-video-production, brand-guidelines]
related_assets: ["brain/brand-assets/oasis-ai/BRAND_SYSTEM.md", "brain/playbooks/youtube_video_pipeline.md", "ad-engine/"]
canon_references: [sutherland-signalling, dunford-positioning]
canon_source: brain/MARKETING_CANON.md
note: "Documentation-only skill. Requires Higgsfield platform access (Seedance) to execute. Per CC's deferred AI capabilities memory, subscription is gated until GPU upgrade — the prompt-craft itself is valuable training before subscribe."
---
...
**OASIS AI brand-lock (when generating OASIS content):** before composing M2 (Studio/Editorial) or M5 (Atmospheric/Empty) scenes for OASIS work, pull the palette from `brain/brand-assets/oasis-ai/BRAND_SYSTEM.md`. Bake the hex values into M5 prompts directly (`#0A1525` bg, `#1FE3F0` cyan, `#11202F` elevated, `#F0F4F8` off-white, `#A8B5C2` body-gray). Never blend OASIS palette with a Hollywood teal-amber unless CC explicitly asks for a remix.

**YouTube pipeline integration:** Seedance output slots into [brain/playbooks/youtube_video_pipeline.md](brain/playbooks/youtube_video_pipeline.md) at **step 3.5** when there's no raw footage to grade. The MP4 from Higgsfield is already graded — skip step 3 (preprocess), proceed directly to step 4 (Remotion intro/outro wrap with `OasisIntroCard` + `OasisOutroCard`).

**Pairing with banana-pro-director:** if CC wants a Seedance shot of a specific recurring character (e.g., his on-camera persona, a PropFlow brand persona, a Nostalgic Requests host), build the character via `banana-pro-director` first (Mode 1 single-image base → Mode 2 6-panel sheet), lock the reference, then write the Seedance prompt against that locked character spec.

**Subscription gate (memory: project_deferred_ai_capabilities.md):** Higgsfield/Seedance subscription is currently gated by CC's GPU upgrade decision. The skill is installed as documentation; do not assume CC has an active Seedance account. If CC asks "actually run this," confirm subscription status first or note the gate.
