...
Maven turns marketing playbooks (e.g., "PMF survey", "cold ad-set launch", "creative refresh") into runnable step lists with verification gates per step. SOPs live in `skills/<area>/SOP_<name>.md` and link out to the skill that owns the step. Used heavily for the vertical-pack rollouts.
